...
- **misspell**: スペルミスの検出
- **gocyclo**: 循環的複雑度の確認（閾値: 15）
- **dupl**: 重複コードの検出（閾値: 100）
...
1. **Lintチェック**: `make lint` でエラーがないこと
2. **テスト実行**: `go test ./...` がすべて成功すること
3. **ビルド確認**: `go build` が成功すること

#### CIでのlint実行

GitHub Actionsでpre-commitフックとして自動的にlintが実行されます：

- **プッシュ時**: `make lint` が自動実行
- **プルリクエスト時**: lint結果がチェックされる
- **失敗時**: マージがブロックされる

#### Pre-commitフック

ローカル環境では `.git/hooks/pre-commit` でコミット前に自動的にlintが実行されます：
...
lintエラーがある場合、コミットが失敗します。`make lint-fix` で自動修正を試みてください。
